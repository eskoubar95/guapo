...
- **Shipmondo pricing unit bridge (M11):** `apps/commerce/src/modules/shipmondo/service.ts` now accepts legacy minor fields (`amount_minor`, `flat_amount_minor`, `price_bands[].amount_minor`) and optional major aliases, but always returns `calculatePrice().calculated_amount` in major units for Medusa cart totals/payment consistency.
