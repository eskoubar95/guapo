...
- **Payments**: Stripe (recurring required for subscriptions). Guapo provider `payment-stripe-guapo`: if payment_collection amount is inflated vs storefront-normalized cart (mixed DKK/øre), PaymentIntent uses corrected total (`compute-payable-major.ts`).
