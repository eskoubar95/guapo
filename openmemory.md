...
## Components
- **Order/invoice PDFs (commerce):** `src/lib/documents/build-order-pdf.ts` renders A4 PDFs with Guapo colours (#051537 / #0f766e), seller block + customer address, meta band (dato, betaling fra `order.metadata` payment_brand/last4 når sat), line-item table (variant.title + product title), totals (subtotal, rabat, fragt, heraf moms når `tax_total` > 0), footer. Seller + legal copy via `GUAPO_INVOICE_*` env (`pdf-seller-config.ts`). **Primært dansk marked** (DKK, moms 25 %, da-tekst når leveringsland DK). `documentKind` `order_confirmation` vs `invoice`; `locale` da/en fra leveringsland. Railway: samme `GUAPO_INVOICE_*`, `PLUNK_*`, `STOREFRONT_URL` på **server + worker** (PDF/mail på worker). Se `apps/commerce/env.template` + `docs/DEPLOY-RAILWAY.md`.
