...
- **Payload Categories (CMS)**: Collection `categories` — faner Content (lokaliseret navn + intro body), SEO (`meta` med plugin-felter title/description/image + Overview), Medusa (read-only `handle` + `medusa_id`). Medusa Admin → Settings → Payload sync **Categories** opretter/opdaterer rækker; `update-payload-categories` opdaterer kun handle/parent, ikke redaktionelt indhold. `seoPlugin` i `payload.config.ts` har `generateTitle` / `generateDescription` (locale-aware) + Live Preview åbner `/{locale}/categories/{handle}` på storefront.
