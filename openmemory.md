...
## Patterns
- **Category PLP (`/[locale]/categories/[handle]`)**: Gyldige handles kommer fra Medusa (`fetchCategoryByHandle`); ukendte handles → `notFound()` (ikke mock-fallback). Produkter kun via `fetchProductsByCategory(medusaCat.id)`. Payload (`fetchPayloadCategoryByHandle`, depth 1) styrer navn, intro-body (Lexical→HTML), SEO meta title/description/OG-billede; `React.cache` deduplikerer fetch i samme request (metadata + page).
