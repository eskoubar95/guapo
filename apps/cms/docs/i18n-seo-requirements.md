...
- ~~**Payload Localization:** Enable built-in Localization~~ **Done:** `payload.config.ts` has `localization: { locales: ['da','en'], defaultLocale: 'da', fallback: true }`. Localized: Pages, Articles, Products (CMS fields), **Categories** (name, slug, meta, body), **Brands** (displayName, meta, body), **ProductTypes** (name, meta, body), Navigation, Footer, Homepage, Media (alt). API: `?locale=da` | `?locale=en`; `?fallback-locale=...` optional. **Payload i18n** (admin UI): `i18n: { supportedLanguages: { da, en }, fallbackLanguage: 'da' }` via `@payloadcms/translations` so editors can use admin in Danish or English. **Note:** Only fields with `localized: true` store separate values per locale; without it, switching locale shows the same value (or fallback). Changing a field to `localized: true` changes stored shape—existing data for that field is lost unless migrated; see [Payload Localization](https://payloadcms.com/docs/configuration/localization).
