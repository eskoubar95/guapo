...
function maxRequests(): number {
  const raw = process.env.SUBSCRIPTIONS_STORE_RATE_LIMIT_MAX?.trim();
...
export function storeSubscriptionsRateLimit(
  req: MedusaRequest,
  res: MedusaResponse,
  next: MedusaNextFunction
): void {
  if (process.env.SUBSCRIPTIONS_STORE_RATE_LIMIT_DISABLED === "true") {
    next();
    return;
  }
  const now = Date.now();
