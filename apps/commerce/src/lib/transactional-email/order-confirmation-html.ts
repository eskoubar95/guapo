...
export function buildOrderConfirmationMoneyBlock(
  ...
  const lineRows = summary.lines
    .map((line) => {
      ...
      const sub = line.subtitle ? `<p style="margin:4px 0 0 0;font-size:13px;line-height:20px;color:#64748b;">${escapeHtml(line.subtitle)}</p>` : "";
  ...
  <img src="${escapeAttr(thumb)}" alt="${escapeAttr(line.title.slice(0, 100))}" width="60" height="60" style="display:block;width:60px;height:60px;border-radius:8px;object-fit:cover;border:1px solid #e2e8f0;">
</td>`
          : "";
    ...
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" class="oc-line-wrap" style="border-collapse:collapse;">
      <tr>
        ...
        <td class="oc-line-desc" style="padding:14px 0 14px 0;vertical-align:top;${descWidthStyle}">
          <p style="margin:0;font-family:Inter,Arial,sans-serif;font-size:15px;line-height:22px;font-weight:600;color:#051537;">${title}</p>
          ${sub}
          <p style="margin:8px 0 0 0;font-family:Inter,Arial,sans-serif;font-size:12px;line-height:18px;color:#94a3b8;">${escapeHtml(qtyLine)}</p>
        </td>
  ...
  return `
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin:20px 0 0 0;border:1px solid #e2e8f0;border-radius:10px;border-collapse:separate;">
  <tbody>
    ${lineRows}
    <tr>
      <td colspan="2" style="padding:16px 16px 14px 16px;background-color:#f8fafc;border-top:1px solid #e2e8f0;">
