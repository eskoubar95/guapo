...
export class Migration20260214232328 extends Migration {
  ...
  override async up(): Promise<void> {
    ...
    this.addSql(`create table if not exists "medusa"."brand" ("id" text not null, "name" text not null, "handle" text not null, "created_at" timestamptz not null default now(), "updated_at" timestamptz not null default now(), "deleted_at" timestamptz null, constraint "brand_pkey" primary key ("id"));`);
  ...
  override async down(): Promise<void> {
    this.addSql(`drop table if exists "medusa"."brand" cascade;`);
